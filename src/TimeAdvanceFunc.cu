...
__global__ void cfd::repair_turbulent_variables(cfd::DZone* zone, cfd::DParameter* param, integer blk_id, integer count_start) {
      ...
      for (int i = 0; i < mx; ++i) {
        if (unphysical(i, j, k)) {
          ...
          if (kn > 0) {
            ...
            updated_var[1] = sv(i, j, k, n_spec + 1) < 0 ? param->limit_flow.sv_inf[n_spec + 1] : sv(i, j, k, n_spec + 1);
          }
...
__global__ void cfd::repair_mixtureFraction_variables(cfd::DZone* zone, cfd::DParameter* param, integer blk_id, integer count_start) {
  const auto mx{ zone->mx }, my{ zone->my }, mz{ zone->mz };
  auto& unphysical = zone->unphysical;
  auto& sv = zone->sv;
