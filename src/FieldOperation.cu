...
__device__ void
cfd::compute_temperature(int i, int j, int k, const cfd::DParameter *param, cfd::DZone *zone, const real *q) {
  const integer n_spec = param->n_spec;
  auto &Y = zone->sv;
  auto &bv = zone->bv;
  ...
  mw = 1 / mw;
  const real gas_const = R_u / mw;
  const real e = q[4] / q[0] - 0.5 * (bv(i, j, k, 1) * bv(i, j, k, 1) + bv(i, j, k, 2) * bv(i, j, k, 2) +
                                      bv(i, j, k, 3) * bv(i, j, k, 3));
