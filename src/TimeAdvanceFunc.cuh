...
void cfd::limit_unphysical_variables(cfd::DZone *zone, cfd::DParameter *param, integer blk_id, int step, dim3 bpg,
  ...
  if constexpr (TurbMethod<turb_method>::label == TurbMethodLabel::SST) {
    test_turbulent_variables<mixture, turb_method> <<< bpg, tpb >>>(zone, param, blk_id, step);
    repair_turbulent_variables <<< 1, 1 >>>(zone, param, blk_id, 0);
  }
  if constexpr (mixture == MixtureModel::FL || mixture == MixtureModel::MixtureFraction) {
    test_mixtureFraction_variables<mixture, turb_method> <<<bpg, tpb >>>(zone, param, blk_id, step);
    repair_mixtureFraction_variables <<< 1, 1 >>>(zone, param, blk_id, 0);
  }
}
...
__global__ void cfd::test_flow(cfd::DZone *zone, cfd::DParameter *param, integer blk_id, int step) {
  ...
  if (unphysical) {
    // try to solve it here, if there are values that are surrounded by bad points, then update the unphysical array to limit it later.
